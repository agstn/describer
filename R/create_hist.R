...
#' @export
create_hist.timePOSIXt <- function(x){
   p <- plotly::plot_ly(width  = 270,
                        height = 60)
   ...
   if(any(i)){
      ...
      ur    <- xr[i]
      tab   <- as.data.frame(table(ur))             # table of unique values & frequencies
      z     <- hms::as_hms(as.character(tab$ur))     # unique values
      prop  <- tab$Freq / length(ur)                # proportion of obs for each unique value
      ...
      # calculate mean
      dm <- rbind(dm, data.frame(Mean=mean(u) %>% hms::as_hms() %>% hms::round_hms(digits=0), n=length(u), miss=miss, y=y))
